...
// ScreenshotCompanion
namespace ScreenshotCompanionCore
{
    [ExecuteInEditMode]
    public class ScreenshotCompanion : MonoBehaviour
    {
        ...
        [HideInInspector]
        public bool foldoutSettings = false;

        [HideInInspector]
        public int lastCamID = 0;
        ...
        public string getSaveDirectory()
        {
            string pickDirectory =
                settings.customDirectoryName != "" ? settings.customDirectoryName : "Screenshots";
        ...
        void clampCutoutBox()
        {
            settings.cutoutPosition.x = Mathf.Clamp(
                settings.cutoutPosition.x,
                settings.cutoutSize.x / 2f,
                (float)Screen.width - settings.cutoutSize.x / 2f
            );
            settings.cutoutPosition.y = Mathf.Clamp(
                settings.cutoutPosition.y,
                settings.cutoutSize.y / 2f,
                (float)Screen.height - settings.cutoutSize.y / 2f
            );
        ...
        void OnGUI()
        {
            ...
            if (
                lastP.x == settings.cutoutPosition.x
                && lastP.y == settings.cutoutPosition.y
                && lastS.x == settings.cutoutSize.x
                && lastS.y == settings.cutoutSize.y
            )
            {
            ...
            if (settings.captureMethod == CaptureMethod.Cutout)
            {
                ...
                GUI.Box(
                    new Rect(
                        settings.cutoutPosition.x - settings.cutoutSize.x / 2f,
                        settings.cutoutPosition.y - settings.cutoutSize.y / 2f,
                        settings.cutoutSize.x,
                        settings.cutoutSize.y
                    ),
                    "",
                    cutoutBoxStyle
                );
        ...
        public IEnumerator CaptureCutout(int id)
        {
            ...
            */
            ...
            var startY = (int)(
                (Screen.height - settings.cutoutPosition.y) - settings.cutoutSize.y / 2f
            );
        ...
        void cutoutEmptyCheck()
        {
            ...
            {
                Debug.Log(
                    "[ScreenshotCompanion] A size of less than 8x8 pixels for Cutout has been detected!"
                );
        ...
        public string getResolution()
        {
            ...
            if (settings.captureMethod == CaptureMethod.RenderTexture)
            {
                return (int)(lastCam.pixelWidth * settings.renderSizeMultiplier)
                    + "x"
                    + (int)(lastCam.pixelHeight * settings.renderSizeMultiplier);
            }

            return lastCam.pixelWidth * settings.captureSizeMultiplier
                + "x"
